...
class Source_03(FlowSpec):
    ...
    @step
    def load(self):
        self.output.to_csv('data/iopa/iopa_source_03_' + now.strftime("%Y_%m_%d_%H%M") + '.csv', index=False)
    ...
    @step
    def data_map(self):
        output_map = self.output.dropna(subset=['latitude', 'longitude'])
        self.html = Plot(output_map).render()
        self.next(self.end)
